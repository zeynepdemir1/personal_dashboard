...
### Ek düzeltmeler (Zeynep'in Aşama 22'yi kullandıktan sonraki geri bildirimi) ✅
Üç ayrı sorun bildirdi, ikisinde netleştirme sorusu sorulup onaylandıktan sonra uygulandı (Zeynep'in "sakın varsayımda bulunup hareket etme" talimatı üzerine):

1. **"Eski programları silemiyorum":** Redis'teki gerçek veri kontrol edildi — tek "eklediğim" (extra) program zaten doğru şekilde silinebiliyordu. Sorun aslında TÜBİTAK 2209-A / TEKNOFEST takım başvurusu / IEEE sempozyumu gibi baştan beri kodda sabit gelen (seed, `data.ts` → `PROGRAMS`) örneklermiş — bunlar bilerek silinemez bırakılmıştı. **Soruldu, "evet silinebilir olsun" onayı alındı.** Yeni `hiddenPrograms: string[]` alanı eklendi (`PersistedState`, diğerleri gibi Redis'te) — "Programı sil" artık seed programlarda da çıkıyor, tıklanınca `hideStaticProgram(title)` çağrılıp o başlık bu listeye ekleniyor; `PROGRAMS` sabiti KODDA değişmiyor, sadece görünümden (Ana Sayfa + Program Takvimi) filtreleniyor — teknik olarak "silme" değil "gizleme", ama kullanıcı için sonuç aynı.
2. **"Ana sayfadan eklendi" yazısı, yazdığım açıklamanın yerini alıyordu:** Gerçek bir görüntüleme hatasıydı — `CalendarScreen.tsx`'te extra programların alt başlığı HER ZAMAN sabit `'Ana sayfadan eklendi'` metniydi, kullanıcının girdiği not asıl satırda hiç görünmüyordu (sadece küçük bir "📝 not" etiketiyle varlığı belli oluyordu). Düzeltildi: `baseNote: p.note || 'Ana sayfadan eklendi'` — not varsa o gösteriliyor, yoksa eski sabit metin (geriye dönük uyumluluk için) kalıyor.
3. **"Program Takvimi'ne eklediğim program takvimde de görünsün, aynı not/tarih/başlıkla":** **Netleştirme sorusu soruldu** (canlı bağlı senkron mu, yoksa tek seferlik bağımsız kopya mı) — **"canlı bağlı kalsın"** onayı alındı: program düzenlenince/silinince takvimdeki görünüm de otomatik güncellenmeli, takvimden ayrıca elle düzenlenip silinememeli. **Teknik çözüm bilerek en basit/sağlam yol seçildi:** ayrı bir kopya `dayNotes` kaydı YARATILMADI (bu, id eşleştirme/senkron mantığı gerektirir, kolayca birbirinden kopabilir) — bunun yerine `DayPanel` ve `MonthView`, o güne denk gelen `extraPrograms` kayıtlarını DOĞRUDAN (her render'da canlı) filtreleyip gösteriyor. Bu sayede "senkron kopması" mimari olarak mümkün DEĞİL — gösterilen veri zaten tek bir kaynaktan (`extraPrograms`) geliyor. DayPanel'de bu girişler ayrı bir stil ve "PROGRAM" etiketiyle işaretleniyor, düzenleme/silme ikonu YOK (sadece "Program Takvimi'nde düzenle →" bağlantısı) — kaynağının Program Takvimi olduğu net. Ay görünümündeki nokta göstergelerine de dahil edildi (o günün notu var gibi görünüyor).

**Uçtan uca doğrulama:** not içeren bir program eklendi → Program Takvimi'nde yazdığı not (sabit metin değil) doğru göründü; bir seed program "Programı sil" ile gizlendi → hem Ana Sayfa hem Program Takvimi'nde kayboldu; program tarihine denk gelen gün paneli açıldı → program orada "PROGRAM" etiketiyle, notuyla birlikte göründü; program Ana Sayfa'dan silindi → gün panelinden de anında kayboldu (ayrı bir kopya olmadığı için senkron kopması ihtimali yok).

## Aşama 23 — Program Keşfi: Güncel Olmayan Sonuçları Ele ✅

**Sorun (Zeynep'in geri bildirimi):** "Senin İçin Bulduklarımız" (Ana Sayfa) ve "Keşfedilen Programlar" sayfasındaki sonuçlarda 2024/2025 yılına ait, artık başvurulamayan/sonuçlanmış programlar görünüyordu. İstek: sadece başvuruya açık VEYA gelecekte açılacak programlar gösterilsin; süresi geçmiş/tamamen bitmiş yarışmalar hiç görünmesin.

- [x] **Kök neden bulundu:** `/api/discover/relevant` zaten süresi geçmiş bir `deadline`'ı olan sonuçları eliyordu (bkz. Aşama 19 madde 2) — ama Ollama (küçük, 8B bir model) çoğu gerçek sonuçtan (ör. "TÜBİTAK 2209-A ... 13 Ekim 2025 ve 12 Kasım 2025 tarihleri arasında") bir tarih ÇIKARAMIYORDU; `deadline: null` kalan sonuçlar bu yüzden SÜRESİZ gösteriliyordu, ne kadar eski olursa olsun. Gerçek veride 375 kayıttan 113'ü "relevant", bunların 111'i tarihsiz.
- [x] **Denenip TERK EDİLEN yaklaşım:** Ollama'ya "bugünün tarihi X, bu güncel mi geçmiş mi?" diye 4. bir soru daha sormak — gerçek örneklerle test edildi (`llama3.1:8b`), model bu bileşik yargıda TUTARSIZ çıktı verdi (aynı örnek çalıştırmalar arasında EVET/GÜNCEL ↔ HAYIR/GEÇMİŞ arasında savruldu), hatta gerçekten güncel/gelecek bir örneği (Mart 2026 tarihli bir hackathon) yanlışlıkla "geçmiş" diye işaretledi. Bu riskin (gerçek bir fırsatı gizleme) kabul edilemez olduğuna karar verildi, Ollama tarafına DOKUNULMADI.
- [x] **Uygulanan çözüm — basit, deterministik bir ikinci sinyal:** `server.js`'e `isStaleByYear()` eklendi — başlık+snippet metninde geçen EN YÜKSEK yıl (20xx deseni) bugünün yılından küçükse, o sonuç muhtemelen geçmiş bir döneme ait kabul ediliyor. `deadline` alanı zaten varsa ona güveniliyor (değişmedi); sadece `deadline: null` olan sonuçlarda bu yeni kontrol devreye giriyor. Hiç yıl geçmeyen sonuçlar (gerçekten süresiz/rutin programlar) etkilenmiyor.
